...
//        conf.setMaster("local[49]")
    ...
    //    ssc.sparkContext.setLogLevel("INFO")    //日志级别

    //指定Kafka  topic 名字
    val topic = MyConfig.KAFKA_TOPIC
    //指定kafka的服务器broker地址，SparkStream的Task直连到kafka的分区上，用底层的API消费，效率更高
    val brokerList = MyConfig.KAFKA_BROKER
    //指定zk的地址，更新消费的偏移量时使用，当然也可以使用Redis和MySQL来记录偏移量 offset
    ...
    kafkaStream.foreachRDD(foreachFunc = kafkaRDD => {
      ...
      if (!kafkaRDD.isEmpty()) {
            ...
            breakable {
              ...
              while (line != null) {
                ...
                if (strings.length >= 3) {
                  ...
                  //                  val hostTpyeXpathInfo = hostXpathInfo.getOrElse(xpathType, new HashSet[String]())
                  //                  hostXpathInfo += xpathType -> hostTpyeXpathInfo
        ...
        val dataRDD: RDD[(String, String, String, String, String, String)] = kafkaRDD.filter(cr => {
          ...
          iter.foreach(cr => {
            ...
            if (!trueRule.equals("") || !faileRule.isEmpty) {
              val transaction: Transaction = redis.multi() // 开始执行事务
              //切换数据库
            ...
            if (xpathMap.contains(host)) {
              ...
              if (content.trim.length >= 10) {
                ...
                if (JavaUtil.isNotEmpty(content)) put.addColumn(Bytes.toBytes("c"), Bytes.toBytes("context"), Bytes.toBytes(content))
        ...
        dataRDD.mapPartitions(f => {
          ...
          val rl = new ListBuffer[mutable.Map[String, String]] // 插入Es, 需要转成map()
          for (t <- f) {
            val url: String = t._1
